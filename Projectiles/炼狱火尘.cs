        ...
        public override void Kill(int timeLeft)
        {
            ...
            if (base.Projectile.owner == Main.myPlayer)
            {
                for (int i = 0; i < 10; i++)
                {
                    ...
                    Projectile.NewProjectile(base.Projectile.Center.X, base.Projectile.Center.Y, (float)(Math.Sin(num4) * 6.0), (float)(Math.Cos(num4) * 6.0), base.Mod.Find<ModProjectile>("炼狱火渣").Type, 405, base.Projectile.knockBack, base.Projectile.owner, 0f, 0f);
                }
            }
            for (int j = 0; j <= 10; j++)
            {
