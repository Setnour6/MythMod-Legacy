...
namespace MythMod.Projectiles
{
    ...
    public class 渊海磷光1 : ModProjectile
	{
		...
		public override void AI()
		{
            ...
            if (Projectile.timeLeft <= 10 && num4 && NPC.CountNPCS(Mod.Find<ModNPC>("星渊水母").Type) > 0 && MythWorld.Myth && Projectile.ai[0] == 1)
            {
                ...
                for (int k = 0; k < 10; k++)
                {
                    ...
                    if ((int)k % 2 == 1)
                    {
                        Projectile.NewProjectile(base.Projectile.Center.X, base.Projectile.Center.Y, (float)Math.Cos(((float)i / 5f) * Math.PI + num7) * 7 / 3 * 1.08f, (float)(0 - (float)Math.Sin(((float)i / 5f) * Math.PI + num7) * 7) / 3 * 1.08f, base.Mod.Find<ModProjectile>("渊海磷光2").Type, base.Projectile.damage / 2, 0.2f, Main.myPlayer, 0f, 0f);
                    }
                }
                num4 = false;
