    ...
    public class 金绿烈焰团 : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int k = 0; k < 30; k++)
            {
                ...
                if((int)k % 2 == 1)
                {
                    Projectile.NewProjectile(base.Projectile.Center.X, base.Projectile.Center.Y, (float)Math.Cos((2 * (float)i / 30) * 3.14159265358979f) * 7 / 3 * 1.44f, (float)(0 - (float)Math.Sin((2 * (float)i / 30) * 3.14159265358979f) * 7) / 3 * 1.44f, base.Mod.Find<ModProjectile>("金绿烈焰团2").Type, base.Projectile.damage / 2, base.Projectile.knockBack, base.Projectile.owner, 0f, 0f);
                }
                else
                {
                    Projectile.NewProjectile(base.Projectile.Center.X, base.Projectile.Center.Y, (float)Math.Cos((2 * (float)i / 30) * 3.14159265358979f) * 7 / 3 * 1.44f, (float)(0 - (float)Math.Sin((2 * (float)i / 30) * 3.14159265358979f) * 7) / 3 * 1.44f, base.Mod.Find<ModProjectile>("金绿烈焰团3").Type, base.Projectile.damage / 2, base.Projectile.knockBack, base.Projectile.owner, 0f, 0f);
                }
            }
        }
    }
