    ...
    public class WaveClub : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (lz % 48 == 1)
            {
                ...
                vk = vk / vk.Length() * 13f;
                int i = Projectile.NewProjectile(p.Center.X + vk.X, p.Center.Y + vk.Y, vk.X, vk.Y, mod.ProjectileType("海浪球"), projectile.damage * 6, projectile.knockBack * 0.5f, p.whoAmI, 0, 0);
            }
            float li = Main.rand.NextFloat(-32f, 32f);
            int num25 = Dust.NewDust(base.projectile.Center - new Vector2(4, 4) + new Vector2(li, li).RotatedBy(projectile.rotation), 0, 0, mod.DustType("Wave"), 0, 0, 0, default(Color), 2f);
