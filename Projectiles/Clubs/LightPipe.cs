...
{
    public class LightPipe : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Math.Abs(lz) < 1)
            {
                lz += Main.rand.Next(-10000, 10000) / 100000f;
            }
            else
            {
                lz *= Main.rand.NextFloat(0.9f,1f);
            }
            Lighting.AddLight(Projectile.Center, lz, lz, lz);
        ...
        {
            spriteBatch.Draw(base.Mod.GetTexture("Projectiles/Clubs/LightPipeGlow"), base.Projectile.Center - Main.screenPosition, null, Color.White * lz, base.Projectile.rotation, new Vector2(32f, 32f), 1f, SpriteEffects.None, 0f);
        }
    }
}
