...
namespace MythMod.Projectiles.projectile4
{
    public class FireBomb : ModProjectile
    {
        ...
        {
            int r = Dust.NewDust(new Vector2(base.Projectile.Center.X, base.Projectile.Center.Y) - new Vector2(4, 4), 0, 3, Mod.Find<ModDust>("Flame3").Type, 0, 0, 0, default(Color), 8f);
            Main.dust[r].noGravity = true;
            for(int i = 0; i < 5; i++)
            {
                Vector2 v = new Vector2(0, 60).RotatedBy(Projectile.timeLeft / 15d);
                int z = Dust.NewDust(new Vector2(base.Projectile.Center.X, base.Projectile.Center.Y) - new Vector2(4, 4) + v, 0, 0, Mod.Find<ModDust>("Flame3").Type, 0, 0, 0, default(Color), 4f);
            ...
            for (int j = 0; j < 200; j++)
            {
                ...
                {
                    if((Main.npc[j].Center - Projectile.Center).Length() < 60)
                    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int i = 0; i < 170; i++)
            {
                ...
                int num5 = Dust.NewDust(new Vector2(base.Projectile.position.X, base.Projectile.position.Y), base.Projectile.width, base.Projectile.height, Mod.Find<ModDust>("Flame").Type, 0f, 0f, 100, Color.White, (float)(6f * Math.Log10(Projectile.damage)));
                Main.dust[num5].velocity = v;
            }
            for (int i = 0; i < 80; i++)
            {
                Vector2 v = new Vector2(0, Main.rand.NextFloat(2.9f, (float)(2.4 * Math.Log10(Projectile.damage)))).RotatedByRandom(Math.PI * 2);
                int num5 = Dust.NewDust(new Vector2(base.Projectile.position.X, base.Projectile.position.Y), base.Projectile.width, base.Projectile.height, Mod.Find<ModDust>("Flame").Type, 0f, 0f, 100, Color.White, (float)(8f * Math.Log10(Projectile.damage)));
