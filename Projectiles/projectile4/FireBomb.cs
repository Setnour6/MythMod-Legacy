...
namespace MythMod.Projectiles.projectile4
{
    public class FireBomb : ModProjectile
    {
        ...
        {
            int r = Dust.NewDust(new Vector2(base.projectile.Center.X, base.projectile.Center.Y) - new Vector2(4, 4), 0, 3, mod.DustType("Flame3"), 0, 0, 0, default(Color), 8f);
            Main.dust[r].noGravity = true;
            for(int i = 0; i < 5; i++)
            {
                Vector2 v = new Vector2(0, 60).RotatedBy(projectile.timeLeft / 15d);
                int z = Dust.NewDust(new Vector2(base.projectile.Center.X, base.projectile.Center.Y) - new Vector2(4, 4) + v, 0, 0, mod.DustType("Flame3"), 0, 0, 0, default(Color), 4f);
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int i = 0; i < 170; i++)
            {
                ...
                int num5 = Dust.NewDust(new Vector2(base.projectile.position.X, base.projectile.position.Y), base.projectile.width, base.projectile.height, mod.DustType("Flame"), 0f, 0f, 100, Color.White, (float)(6f * Math.Log10(projectile.damage)));
                Main.dust[num5].velocity = v;
            }
            for (int i = 0; i < 80; i++)
            {
                Vector2 v = new Vector2(0, Main.rand.NextFloat(2.9f, (float)(2.4 * Math.Log10(projectile.damage)))).RotatedByRandom(Math.PI * 2);
                int num5 = Dust.NewDust(new Vector2(base.projectile.position.X, base.projectile.position.Y), base.projectile.width, base.projectile.height, mod.DustType("Flame"), 0f, 0f, 100, Color.White, (float)(8f * Math.Log10(projectile.damage)));
