    ...
    public class RedCrack : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int y = 0; y < Times; y++)
            {
                ...
                spriteBatch.Draw(TextureAssets.Projectile[Projectile.type].Value, v1 - Main.screenPosition, new Rectangle(0, x, 10, 10 - 2 * x), new Color(lig * (Times - y) / 50f, lig * (Times - y) / 50f, lig * (Times - y) / 50f, 0), Rot2, new Vector2(5, 5 - x), Projectile.scale, SpriteEffects.None, 0f);
