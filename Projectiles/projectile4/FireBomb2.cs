...
namespace MythMod.Projectiles.projectile4
{
    public class FireBomb2 : ModProjectile
    {
        ...
        {
            int r = Dust.NewDust(new Vector2(base.Projectile.Center.X, base.Projectile.Center.Y) - new Vector2(4, 4), 0, 3, Mod.Find<ModDust>("Flame3").Type, 0, 0, 0, default(Color), 10f);
            Main.dust[r].noGravity = true;
            for(int i = 0; i < 5; i++)
            {
                Vector2 v = new Vector2(0, 80).RotatedBy(Projectile.timeLeft / 20d);
                int z = Dust.NewDust(new Vector2(base.Projectile.Center.X, base.Projectile.Center.Y) - new Vector2(4, 4) + v, 0, 0, Mod.Find<ModDust>("Flame3").Type, 0, 0, 0, default(Color), 5f);
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int i = 0; i < 170; i++)
            {
                ...
                int num5 = Dust.NewDust(new Vector2(base.Projectile.position.X, base.Projectile.position.Y), base.Projectile.width, base.Projectile.height, Mod.Find<ModDust>("Flame").Type, 0f, 0f, 100, Color.White, (float)(7f * Math.Log10(Projectile.damage)));
