...
namespace MythMod.Projectiles.projectile4
{
    public class FireBomb3 : ModProjectile
    {
        ...
        {
            int r = Dust.NewDust(new Vector2(base.projectile.Center.X, base.projectile.Center.Y) - new Vector2(4, 4), 0, 3, mod.DustType("Flame3"), 0, 0, 0, default(Color), 15f);
            Main.dust[r].noGravity = true;
            for(int i = 0; i < 5; i++)
            {
                Vector2 v = new Vector2(0, 135).RotatedBy(projectile.timeLeft / 30d);
                int z = Dust.NewDust(new Vector2(base.projectile.Center.X, base.projectile.Center.Y) - new Vector2(4, 4) + v, 0, 0, mod.DustType("Flame3"), 0, 0, 0, default(Color), 7f);
        ...
        public override void Kill(int timeLeft)
        {
            ...
            {
                Vector2 v = new Vector2(0, Main.rand.NextFloat(5.8f, (float)(4.8 * Math.Log10(projectile.damage)))).RotatedByRandom(Math.PI * 2);
                int num5 = Dust.NewDust(new Vector2(base.projectile.position.X, base.projectile.position.Y), base.projectile.width, base.projectile.height, mod.DustType("Flame"), 0f, 0f, 100, Color.White, (float)(12f * Math.Log10(projectile.damage)));
