...
{
    public class RainbowFlashlight : ModProjectile
    {

        public override void SetStaticDefaults()
        {
            // base.DisplayName.SetDefault("彩虹手电筒");
        ...
        public override void AI()
        {
            ...
            if(mplayer.SD2 > 0 && mplayer.SD == 15)
            {
                base.Projectile.timeLeft = 2;
            }
            ...
            if (Projectile.ai[0] == 0)
            {
                ...
                {
                    Lighting.AddLight(base.Projectile.Center + step * Projectile.velocity, (float)(255 - base.Projectile.alpha) / 255f * (30 - (float)step) / 30f * Lig * (float)Math.Sin(step / 30d + Main.time / 30d + Math.PI * 2) * 1.5f, (float)(255 - base.Projectile.alpha) / 255f * (30 - (float)step) / 30f * Lig * (float)Math.Sin(step / 30d + Main.time / 30d + Math.PI * 4 / 3d) * 1.5f, (float)(255 - base.Projectile.alpha) / 255f * (30 - (float)step) / 30f * Lig * (float)Math.Sin(step / 30d + Main.time / 30d + Math.PI * 2 / 3d) * 1.5f);
                }
            }
            if(Main.mouseLeft)
            {
