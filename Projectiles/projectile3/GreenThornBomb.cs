    ...
    public class GreenThornBomb : ModProjectile
	{
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int j = 0; j < 15; j++)
            {
                ...
                int zi = Projectile.NewProjectile(base.projectile.Center.X, base.projectile.Center.Y, v.X, v.Y, mod.ProjectileType("Cyanline"), (int)((double)base.projectile.damage), base.projectile.knockBack, base.projectile.owner, 0f, 0f);
            }
            int num10 = 0;
            for (int i = 0; i < 90; i++)
            {
                Vector2 v = new Vector2(0, Main.rand.NextFloat(2.9f, (float)(2.4 * Math.Log10(projectile.damage)))).RotatedByRandom(Math.PI * 2);
                int num5 = Dust.NewDust(new Vector2(base.projectile.position.X, base.projectile.position.Y), base.projectile.width, base.projectile.height, mod.DustType("Poison"), 0f, 0f, 100, Color.White, (float)(4f * Math.Log10(projectile.damage)));
            ...
            for (int i = 0; i < 60; i++)
            {
                ...
                int num5 = Dust.NewDust(new Vector2(base.projectile.position.X, base.projectile.position.Y) + v * 20f, base.projectile.width, base.projectile.height, mod.DustType("Poison"), 0f, 0f, 100, Color.White, (float)(12f * Math.Log10(projectile.damage)));
