...
namespace MythMod.Projectiles
{
    ...
    public class GlitterRay3 : ModProjectile
	{
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                Projectile.NewProjectile(base.Projectile.Center.X, base.Projectile.Center.Y, (float)Math.Cos(((float)i / 3f) * Math.PI + num7) * 7 / 3 * 2.88f, (float)(0 - (float)Math.Sin(((float)i / 3f) * Math.PI + num7) * 7) / 3 * 2.88f, base.Mod.Find<ModProjectile>("灿金射线4").Type, base.Projectile.damage / 2, 0.2f, Main.myPlayer, 0f, 0f);
            }
        }
