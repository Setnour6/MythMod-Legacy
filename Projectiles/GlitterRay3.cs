...
namespace MythMod.Projectiles
{
    ...
    public class GlitterRay3 : ModProjectile
	{
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                Projectile.NewProjectile(base.projectile.Center.X, base.projectile.Center.Y, (float)Math.Cos(((float)i / 3f) * Math.PI + num7) * 7 / 3 * 2.88f, (float)(0 - (float)Math.Sin(((float)i / 3f) * Math.PI + num7) * 7) / 3 * 2.88f, base.mod.ProjectileType("灿金射线4"), base.projectile.damage / 2, 0.2f, Main.myPlayer, 0f, 0f);
            }
        }
