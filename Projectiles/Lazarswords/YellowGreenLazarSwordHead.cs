...
{
    public class YellowGreenLazarSwordHead : ModProjectile
    {

        public override void SetStaticDefaults()
        {
            base.DisplayName.SetDefault("黄绿激光刃");
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            if (l != 0)
            {
                ...
                int num = base.Mod.GetTexture("Projectiles/Lazarswords/YellowGreenLazarSwordLazar").Height / Main.projFrames[base.Projectile.type];
                int y = num * base.Projectile.frame;
                for (float i = 0; i < maxDistance; i += step)
                {
                    Texture2D texture;
                    texture = base.Mod.GetTexture("Projectiles/Lazarswords/YellowGreenLazarSwordLazar");
                    if (i > 0)
                    {
                        ...
                        spriteBatch.Draw(base.Mod.GetTexture("Projectiles/Lazarswords/YellowGreenLazarSwordLazar2"), base.Projectile.Center + unit * (i) - Main.screenPosition, new Rectangle?(new Rectangle(0, 0, 32, 32)), new Color(20, 20, 20, 0), r - (float)Math.PI / 4f, TextureAssets.Projectile[Projectile.type].Value.Size() * 0.5f, 1f, SpriteEffects.None, 0f);
                    }
                    ...
                    if (maxDis < 80f + Dx)
                    {
                        Vector2 vi = new Vector2(0, Main.rand.NextFloat(0, 3)).RotatedByRandom(Math.PI * 2);
                        int dustID = Dust.NewDust(base.Projectile.Center + unit * (maxDis - 42), 20, 20, Mod.Find<ModDust>("Olivine").Type, vi.X, vi.Y, 201, Color.Navy, 3.5f);/*粉尘效果不用管*/
                ...
                if (drawN)
                {
                    for (float i = maxDistance; i < maxDistance + 38; i += 2)
                    {
                        ...
                        if (i > 0)
                        {
                            ...
                            spriteBatch.Draw(base.Mod.GetTexture("Projectiles/Lazarswords/YellowGreenLazarSwordLazar2"), base.Projectile.Center + unit * (i + 1) - Main.screenPosition, new Rectangle?(new Rectangle(0, 0, 32, 32)), new Color(20, 20, 20, 0), r - (float)Math.PI / 4f, TextureAssets.Projectile[Projectile.type].Value.Size() * 0.5f, 1 - (i - maxDistance) / 46f, SpriteEffects.None, 0f);
                        }
                        float[] samples = new float[2];
