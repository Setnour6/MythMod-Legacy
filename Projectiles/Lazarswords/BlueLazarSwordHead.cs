    ...
    public class BlueLazarSwordHead : ModProjectile
    {
            ...
            if (l != 0)
            {
                ...
                for (float i = 0; i < maxDistance; i += step)
                {
                    ...
                    if (i > 0)
                    {
                        ...
                        spriteBatch.Draw(base.mod.GetTexture("Projectiles/Lazarswords/BlueLazarSwordLazar2"), base.projectile.Center + unit * (i) - Main.screenPosition, new Rectangle?(new Rectangle(0, 0, 32, 32)), new Color(20, 20, 20, 0), r - (float)Math.PI / 4f, Main.projectileTexture[projectile.type].Size() * 0.5f, 1f, SpriteEffects.None, 0f);
                    }
                    ...
                    if (maxDis < 80f + Dx)
                    {
                        ...
                        if(Main.rand.Next(6) == 1)
                        {
                            ...
                            int num5 = Projectile.NewProjectile(base.projectile.Center.X + unit.X * (maxDis - 42), base.projectile.Center.Y + unit.Y * (maxDis - 42), (float)num2 * projectile.scale * 1.5f, (float)num3 * projectile.scale * 1.5f, base.mod.ProjectileType("BlueGemDust2"), (int)((double)base.projectile.damage * 0.1f), base.projectile.knockBack, base.projectile.owner, 0f, 0f);
                    ...
                    for (float i = maxDistance; i < maxDistance + 38; i += 2)
                    {
                        ...
                        if (i > maxDis - 38)
                        {
                            ...
                            for (int c = 0; c <= 2; c++)
                            {
                                ...
                                int num5 = Projectile.NewProjectile(base.projectile.Center.X + unit.X * (i - 42), base.projectile.Center.Y + unit.Y * (i - 42), (float)num2 * projectile.scale * 1.5f, (float)num3 * projectile.scale * 1.5f, base.mod.ProjectileType("BlueGemDust2"), (int)((double)base.projectile.damage * 0.1f), base.projectile.knockBack, base.projectile.owner, 0f, 0f);
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
        {
            ...
            for (int c = 0; c <= 4; c++)
            {
                ...
                double num2 = Math.Sin((double)num1 * Math.PI) * num4 / 360f;
                double num3 = Math.Cos((double)num1 * Math.PI) * num4 / 360f;
                int num5 = Projectile.NewProjectile(targetHitbox.Center.X, targetHitbox.Center.Y, (float)num2 * projectile.scale * 1.5f, (float)num3 * projectile.scale * 1.5f, base.mod.ProjectileType("BlueGemDust2"), (int)((double)base.projectile.damage * 0.1f), base.projectile.knockBack, base.projectile.owner, 0f, 0f);
                Main.projectile[num5].scale = Main.rand.Next(1150, 2200) / 2000f;
            }
        }
    }
