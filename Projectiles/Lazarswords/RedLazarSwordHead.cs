            ...
            if (l != 0)
            {
                ...
                for (float i = 0; i < maxDistance; i += step)
                {
                    ...
                    if (i > 0)
                    {
                        ...
                        spriteBatch.Draw(base.Mod.GetTexture("Projectiles/Lazarswords/RedLazarSwordLazar2"), base.Projectile.Center + unit * (i) - Main.screenPosition, new Rectangle?(new Rectangle(0, 0, 32, 32)), new Color(20, 20, 20, 0), r - (float)Math.PI / 4f, TextureAssets.Projectile[Projectile.type].Value.Size() * 0.5f, 1f, SpriteEffects.None, 0f);
                    }
                    ...
                    if (maxDis < 80f + Dx)
                    {
                        ...
                        if (Main.rand.Next(6) == 1)
                        {
                            ...
                            int num5 = Projectile.NewProjectile(base.Projectile.Center.X + unit.X * (maxDis - 42), base.Projectile.Center.Y + unit.Y * (maxDis - 42), (float)num2 * Projectile.scale * 1.5f, (float)num3 * Projectile.scale * 1.5f, base.Mod.Find<ModProjectile>("RedGemDust2").Type, (int)((double)base.Projectile.damage * 0.1f), base.Projectile.knockBack, base.Projectile.owner, 0f, 0f);
                    ...
                    for (float i = maxDistance; i < maxDistance + 38; i += 2)
                    {
                        ...
                        if (i > maxDis - 38)
                        {
                            ...
                            for (int c = 0; c <= 2; c++)
                            {
                                ...
                                int num5 = Projectile.NewProjectile(base.Projectile.Center.X + unit.X * (i - 42), base.Projectile.Center.Y + unit.Y * (i - 42), (float)num2 * Projectile.scale * 1.5f, (float)num3 * Projectile.scale * 1.5f, base.Mod.Find<ModProjectile>("RedGemDust2").Type, (int)((double)base.Projectile.damage * 0.1f), base.Projectile.knockBack, base.Projectile.owner, 0f, 0f);
