            ...
            if (l != 0)
            {
                ...
                for (float i = 0; i < maxDistance; i += step)
                {
                    ...
                    if (i > 0)
                    {
                        ...
                        spriteBatch.Draw(base.mod.GetTexture("Projectiles/Lazarswords/PurpleLazarSwordLazar2"), base.projectile.Center + unit * (i) - Main.screenPosition, new Rectangle?(new Rectangle(0, 0, 32, 32)), new Color(20, 20, 20, 0), r - (float)Math.PI / 4f, Main.projectileTexture[projectile.type].Size() * 0.5f, 1f, SpriteEffects.None, 0f);
                    }
