...
{
    public class ResplendentMirror : ModProjectile
    {

        public override void SetStaticDefaults()
        {
            base.DisplayName.SetDefault("璀璨镜盾");
        ...
        public override void AI()
        {
            ...
            for(int i = 0;i < 1000;i++)
            {
                if(!Main.projectile[i].friendly)
                {
                    ...
                    {
                        if ((Main.projectile[i].Center + Main.projectile[i].velocity - p.Center).Length() <= 48)
                        {
                            Main.projectile[i].friendly = true;
                            Main.projectile[i].velocity *= -1 ;
                            Main.projectile[i].position += Main.projectile[i].velocity * 1.2f;
                        }
                        if ((Main.projectile[i].Center + Main.projectile[i].velocity - p.Center).Length() <= 90)
                        {
