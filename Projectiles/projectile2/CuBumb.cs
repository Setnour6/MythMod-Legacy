...
namespace MythMod.Projectiles.projectile2
{
    ...
    public class CuBumb : ModProjectile
	{
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int j = 0; j < 20; j++)
            {
                Vector2 v = new Vector2(0, 10).RotatedByRandom(Math.PI * 2) * Main.rand.Next(0, 2000) / 1600f;
                Projectile.NewProjectile(base.Projectile.Center.X, base.Projectile.Center.Y, v.X, v.Y, base.Mod.Find<ModProjectile>("CuCoin").Type, (int)((double)base.Projectile.damage * 0.2f), base.Projectile.knockBack, base.Projectile.owner, 0f, 0f);
            }
            for (int k = 0; k <= 10; k++)
            {
                Vector2 v = new Vector2(0, 40).RotatedByRandom(Math.PI * 2);
                int num4 = Projectile.NewProjectile(base.Projectile.Center.X + v.X, base.Projectile.Center.Y + v.Y, 0, 0, base.Mod.Find<ModProjectile>("CuFlame").Type, base.Projectile.damage, base.Projectile.knockBack, base.Projectile.owner, 0f, 0f);
            }
            for (int i = 0; i <= 25; i++)
            {
                ...
                int num5 = Projectile.NewProjectile(base.Projectile.Center.X, base.Projectile.Center.Y, (float)num2, (float)num3, base.Mod.Find<ModProjectile>("CuDust").Type, (int)((double)base.Projectile.damage * 0.1f), base.Projectile.knockBack, base.Projectile.owner, 0f, 0f);
