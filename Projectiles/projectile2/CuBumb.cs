    ...
    public class CuBumb : ModProjectile
	{
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int j = 0; j < 20; j++)
            {
                ...
                Projectile.NewProjectile(base.projectile.Center.X, base.projectile.Center.Y, v.X, v.Y, base.mod.ProjectileType("CuCoin"), (int)((double)base.projectile.damage * 0.2f), base.projectile.knockBack, base.projectile.owner, 0f, 0f);
            }
            for (int k = 0; k <= 10; k++)
            {
                Vector2 v = new Vector2(0, 40).RotatedByRandom(Math.PI * 2);
                int num4 = Projectile.NewProjectile(base.projectile.Center.X + v.X, base.projectile.Center.Y + v.Y, 0, 0, base.mod.ProjectileType("CuFlame"), base.projectile.damage, base.projectile.knockBack, base.projectile.owner, 0f, 0f);
            }
            for (int i = 0; i <= 25; i++)
            {
                ...
                int num5 = Projectile.NewProjectile(base.projectile.Center.X, base.projectile.Center.Y, (float)num2, (float)num3, base.mod.ProjectileType("CuDust"), (int)((double)base.projectile.damage * 0.1f), base.projectile.knockBack, base.projectile.owner, 0f, 0f);
