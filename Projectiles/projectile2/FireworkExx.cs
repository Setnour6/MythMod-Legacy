...
namespace MythMod.Projectiles.projectile2
{
    public class FireworkExx : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            if (projectile.ai[0] == 0)
            {
                Projectile.NewProjectile(base.projectile.Center.X, base.projectile.Center.Y, 0, 0, base.mod.ProjectileType("FireworkStop"), 0, 0, base.projectile.owner, 0f, 0f);
            }
            ...
            switch (Main.rand.Next(1, 128))
            {
                ...
                case 127:
                    for (int j = 0; j < 50; j++)
                    {
                        ...
                        float m = (float)Main.rand.Next(0, 15000);
                        float l = (float)Main.rand.Next((int)m, 50000) / 10000f;
                        Projectile.NewProjectile(base.projectile.Center.X, base.projectile.Center.Y, (float)((float)l * Math.Cos((float)a)), (float)((float)l * Math.Sin((float)a)), base.mod.ProjectileType("FireworkSliverShineBreak"), base.projectile.damage / 20, base.projectile.knockBack, base.projectile.owner, 0f, 0f);
                    }
                    break;
            }
        }
