...
{
    public class FireworkExx : ModProjectile
    {
        private bool x = true;
        private float i2 = 0;
        private float j2 = 0;
        ...
        public override void Kill(int timeLeft)
        {
            ...
            if (Projectile.ai[0] == 0)
            {
                Projectile.NewProjectile(base.Projectile.Center.X, base.Projectile.Center.Y, 0, 0, base.Mod.Find<ModProjectile>("FireworkStop").Type, 0, 0, base.Projectile.owner, 0f, 0f);
            }
            ...
            switch (Main.rand.Next(1, 128))
            {
                ...
                case 127:
                    for (int j = 0; j < 50; j++)
                    {
                        ...
                        float m = (float)Main.rand.Next(0, 15000);
                        float l = (float)Main.rand.Next((int)m, 50000) / 10000f;
                        Projectile.NewProjectile(base.Projectile.Center.X, base.Projectile.Center.Y, (float)((float)l * Math.Cos((float)a)), (float)((float)l * Math.Sin((float)a)), base.Mod.Find<ModProjectile>("FireworkSliverShineBreak").Type, base.Projectile.damage / 20, base.Projectile.knockBack, base.Projectile.owner, 0f, 0f);
                    }
                    break;
            }
        }
