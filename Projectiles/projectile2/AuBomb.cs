    ...
    public class AuBomb : ModProjectile
	{
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int j = 0; j < 32; j++)
            {
                ...
                Projectile.NewProjectile(base.projectile.Center.X, base.projectile.Center.Y, v.X, v.Y, base.mod.ProjectileType("AuCoin"), (int)((double)base.projectile.damage * 0.2f), base.projectile.knockBack, base.projectile.owner, 0f, 0f);
            }
            for (int k = 0; k <= 10; k++)
            {
                Vector2 v = new Vector2(0, 40).RotatedByRandom(Math.PI * 2);
                int num4 = Projectile.NewProjectile(base.projectile.Center.X + v.X, base.projectile.Center.Y + v.Y, 0, 0, base.mod.ProjectileType("AuFlame"), base.projectile.damage, base.projectile.knockBack, base.projectile.owner, 0f, 0f);
            }
            for (int i = 0; i <= 32; i++)
            {
                ...
                int num5 = Projectile.NewProjectile(base.projectile.Center.X, base.projectile.Center.Y, (float)num2, (float)num3, base.mod.ProjectileType("AuDust"), (int)((double)base.projectile.damage * 0.1f), base.projectile.knockBack, base.projectile.owner, 0f, 0f);
