...
namespace MythMod.Projectiles.projectile2
{
    ...
    public class AgBomb : ModProjectile
	{
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int j = 0; j < 20; j++)
            {
                Vector2 v = new Vector2(0, 10).RotatedByRandom(Math.PI * 2) * Main.rand.Next(0, 2000) / 1000f;
            ...
            for (int k = 0; k <= 10; k++)
            {
                ...
                int num4 = Projectile.NewProjectile(base.Projectile.Center.X + v.X, base.Projectile.Center.Y + v.Y, 0, 0, base.Mod.Find<ModProjectile>("AgFlame").Type, base.Projectile.damage, base.Projectile.knockBack, base.Projectile.owner, 0f, 0f);
            }
            for (int i = 0; i <= 28; i++)
            {
                float num4 = (float)(Main.rand.Next(500, 8000)) * ((600 - timeLeft) / 600f + 0.4f);
                double num1 = Main.rand.Next(0, 1000) / 500f;
                double num2 = Math.Sin((double)num1 * Math.PI) * num4 / 136f;
                double num3 = Math.Cos((double)num1 * Math.PI) * num4 / 136f;
                int num5 = Projectile.NewProjectile(base.Projectile.Center.X, base.Projectile.Center.Y, (float)num2, (float)num3, base.Mod.Find<ModProjectile>("AgDust").Type, (int)((double)base.Projectile.damage * 0.1f), base.Projectile.knockBack, base.Projectile.owner, 0f, 0f);
