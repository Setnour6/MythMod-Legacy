...
{
    public class BStarIII : ModItem
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("三阶魔法落星符咒");
        ...
        public override bool CanUseItem(Player player)
        {
            ...
            if(mplayer.MagicCool == 0)
            {
                for(int t = 0;t < 45;t++)
                {
                    ...
                    Vector2 v3 = new Vector2(0, 10).RotatedBy(Main.rand.NextFloat(-0.84f,0.84f) * Main.rand.NextFloat(0f,1.2f)) * Main.rand.NextFloat(0.4f, 1.2f);
                ...
                {
                    Vector2 v1 = Main.screenPosition + new Vector2(Main.mouseX, Main.mouseY);
                    Vector2 v2 = new Vector2(0,8).RotatedBy(t * Math.PI / 6d);
                    int y = Projectile.NewProjectile(v1.X, v1.Y, v2.X, v2.Y, Mod.Find<ModProjectile>("BlueStar").Type, Item.damage, 0.5f, Main.myPlayer, 10f, 25f);
                    Main.projectile[y].scale = Main.rand.NextFloat(0.9f, 1.3f);
                    Main.projectile[y].damage = (int)(Item.damage * Main.projectile[y].scale);
                }
