...
namespace MythMod.Items.Weapons
{
    public class NavyLazarSword : ModItem
    {
        ...
        public override bool? UseItem(Player player)/* tModPorter Suggestion: Return null instead of false */
        {
            if (o == 0)
            {
                Projectile.NewProjectile(player.Center.X, player.Center.Y, 0, 0, Mod.Find<ModProjectile>("NavyLazarSwordHead").Type, (int)(Item.damage * player.GetDamage(DamageClass.Melee)), Item.knockBack, Main.myPlayer, 0f, 0f);
                o += 1;
            }
            if (!Main.mouseLeft)
            {
