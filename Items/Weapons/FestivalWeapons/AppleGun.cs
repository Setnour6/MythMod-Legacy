...
namespace MythMod.Items.Weapons.FestivalWeapons
{
    public class AppleGun : ModItem
	{
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            {
                Projectile.NewProjectile(position.X, position.Y, speedX, speedY, base.Mod.Find<ModProjectile>("AppleGun").Type, (int)((double)damage * 2f), knockBack * 5, player.whoAmI, 0, 0f);
            }
            return true;
        }
    }
