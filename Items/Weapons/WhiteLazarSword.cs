...
namespace MythMod.Items.Weapons
{
    public class WhiteLazarSword : ModItem
    {
        ...
        public override bool UseItem(Player player)
        {
            if (o == 0)
            {
                Projectile.NewProjectile(player.Center.X, player.Center.Y, 0, 0, mod.ProjectileType("WhiteLazarSwordHead"), (int)(item.damage * player.meleeDamage), item.knockBack, Main.myPlayer, 0f, 0f);
                o += 1;
            }
            if (!Main.mouseLeft)
            {
