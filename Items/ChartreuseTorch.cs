...
{
	public class ChartreuseTorch : ModItem
	{
		public override void SetDefaults()
		{
			...
			Item.createTile = Mod.Find<ModTile>("ChartreuseTorch").Type;
			Item.flame = true;
			Item.value = 50;
		}
		...
		public override void HoldItem(Player player)
		{
			...
			{
				int num = Dust.NewDust(new Vector2(player.itemLocation.X + 16f * player.direction, player.itemLocation.Y - 14f * player.gravDir), 4, 4, Mod.Find<ModDust>("Olivine").Type);
                Main.dust[num].velocity.Y = -2.5f;
			}
			Vector2 position = player.RotatedRelativePoint(new Vector2(player.itemLocation.X + 12f * player.direction + player.velocity.X, player.itemLocation.Y - 14f + player.velocity.Y), true);
			Lighting.AddLight(position, 0.25f, 0.65f, 0f);
		}
		...
		public override void AddRecipes()
		{
			...
			recipe.AddIngredient(null, "Olivine");
			recipe.Register();
		}
	}
}