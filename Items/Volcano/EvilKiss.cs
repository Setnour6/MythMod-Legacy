...
namespace MythMod.Items.Volcano
{
	public class EvilKiss : ModItem
	{
        ...
        public override void OnHitNPC(Player player, NPC target, int damage, float knockback, bool crit)
        {
            ...
            for(int i = 0;i < 15;i++)
            {
                Dust.NewDust(new Vector2(target.position.X, target.position.Y), target.width, target.height, Mod.Find<ModDust>("Flame").Type, 0f, 0f, 0, default(Color), 1f);
                Dust.NewDust(new Vector2(target.position.X, target.position.Y), target.width, target.height, Mod.Find<ModDust>("Flame").Type, 0f, 0f, 0, default(Color), 1.5f);
            }
            ...
            if(Main.rand.Next(10) == 1)
            {
                ...
                for (int k = 0; k <= 10; k++)
                {
                    ...
                    int num4 = Projectile.NewProjectile(target.Center.X, target.Center.Y, (float)((float)l * Math.Cos((float)a)) * 0.36f, (float)((float)l * Math.Sin((float)a)) * 0.36f, base.Mod.Find<ModProjectile>("火山溅射").Type, damage, 0, Main.myPlayer, 0f, 0f);
