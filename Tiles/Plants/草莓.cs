...
namespace MythMod.Tiles.Plants
{
    ...
    public class 草莓 : ModTile
    {
        ...
        public override void KillMultiTile(int i, int j, int frameX, int frameY)
        {
            if (frameX == 132)
            {
                Item.NewItem(i * 16, j * 16, 16, 32, base.Mod.Find<ModItem>("草莓").Type);
                Item.NewItem(i * 16, j * 16, 16, 32, base.Mod.Find<ModItem>("草莓").Type);
                Item.NewItem(i * 16, j * 16, 16, 32, base.Mod.Find<ModItem>("草莓").Type);
                Item.NewItem(i * 16, j * 16, 16, 32, base.Mod.Find<ModItem>("草莓").Type);
            }
        ...
        {
            if (Main.tile[i, j].TileFrameX < 190 && Main.rand.Next(2) == 1)
            {
                for (int y = j; y < j + 4; y++)
                {
                    if (Main.tile[i, y - 2].TileType == Mod.Find<ModTile>("草莓").Type)
                    {
                        Main.tile[i, y - 2].TileFrameX += 64;
                    }
                }
                xm += 1;
            }
