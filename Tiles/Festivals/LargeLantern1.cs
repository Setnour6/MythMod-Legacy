...
namespace MythMod.Tiles.Festivals
{
	public class LargeLantern1 : ModTile
	{
		...
		{
            if(Main.tile[i, j].TileFrameX < 40 && Main.tile[i, j].TileFrameY < 48)
            {
        ...
        {
            Item.NewItem(i * 16, j * 16, 16, 32, base.Mod.Find<ModItem>("LargeLantern1").Type);
        }
        public override void HitWire(int i, int j)
        {
            ...
            for (int k = num; k < num + 3; k++)
            {
                for (int l = num2; l < num2 + 6; l++)
                {
                    ...
                    {
                        if (Main.tile[k, l].TileFrameX < 48)
                        {
