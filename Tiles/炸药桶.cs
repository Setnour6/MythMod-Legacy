...
namespace MythMod.Tiles
{
	...
	public class 炸药桶 : ModTile
	{
		...
		{
			Item.NewItem(i * 16, j * 16, 16, 16, base.Mod.Find<ModItem>("TNTBoom").Type, 1, false, 0, false, false);
		}
			...
			for (int k = num; k < num + 2; k++)
			{
				for (int l = num2; l < num2 + 2; l++)
				{
					...
					if (Main.tile[k, l].HasTile && Main.tile[k, l].TileType == base.Type)
					{
                        ...
                        Projectile.NewProjectile(i * 16 + 112, j * 16 - 40 - 112, 0, -2, 108, 600, 0, Main.myPlayer, 10, 0f);
                        Projectile.NewProjectile(i * 16 - 112, j * 16 - 40 - 112, 0, -2, 108, 600, 0, Main.myPlayer, 10, 0f);
                        SoundEngine.PlaySound(Mod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/烟花爆炸"), i * 16, j *16);
                        for (int k2 = 0; k2 <= 10; k2++)
                        {
                            ...
                            int num4 = Projectile.NewProjectile(i * 16, j * 16, (float)((float)l2 * Math.Cos((float)a)) * 0.36f, (float)((float)l * Math.Sin((float)a)) * 0.36f, base.Mod.Find<ModProjectile>("火山溅射").Type, 200, 10, Main.myPlayer, 0f, 0f);
                            Main.projectile[num4].scale = (float)Main.rand.Next(7000, 13000) / 10000f;
                        }
                        for (int k2 = 0; k2 <= 10; k2++)
                        {
                            Vector2 v = new Vector2(0, 40).RotatedByRandom(Math.PI * 2);
                            int num4 = Projectile.NewProjectile(i * 16 + v.X, j * 16 + v.Y, 0, 0, base.Mod.Find<ModProjectile>("爆炸效果光").Type, 600, 10, Main.myPlayer, 0f, 0f);
                            Projectile.NewProjectile(i * 16, j * 16, 0, 0, base.Mod.Find<ModProjectile>("火球冲击波").Type, 0, 0, Main.myPlayer, 0f, 0f);
                        }
