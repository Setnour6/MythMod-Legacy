...
namespace MythMod.NPCs.FinalEye
{
    ...
    public class WeatherStone : ModNPC
	{
		...
		public override void AI()
		{
			...
			if(base.NPC.localAI[0] >= 2260f && base.NPC.localAI[0] <= 2560f && base.NPC.localAI[0] % 2 == 0 && M < 1f)
	    	{
				M += 0.02f;
			}
			if(base.NPC.localAI[0] >= 2880f && base.NPC.localAI[0] <= 3200f && base.NPC.localAI[0] % 2 == 0 && M > 0.05f)
	    	{
				M -= 0.04f;
			}
	    	if(base.NPC.localAI[0] >= 2400f && base.NPC.localAI[0] <= 2880f && base.NPC.localAI[0] % 60 == 0)
	    	{
				...
				if(num4 <= 80)
				{
					...
					for (int a = 0 ; a < 6 ; a++)
					{
		                ...
		                int num34 = Projectile.NewProjectile(base.NPC.Center.X, base.NPC.Center.Y, -Speed3.X, -Speed3.Y, base.Mod.Find<ModProjectile>("气旋").Type, 555, 2f, Main.myPlayer, 0f, 0f);
                        Main.projectile[num34].timeLeft = 600;
						Main.projectile[num34].scale = 0.2f;
					}
				}
				if(num4 > 80)
				{
		...
		public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
		{
            ...
            vector2 -= new Vector2((float)base.Mod.GetTexture("NPCs/终天灭世眼/天气封印石_Glow").Width, (float)(base.Mod.GetTexture("NPCs/终天灭世眼/天气封印石_Glow").Height / Main.npcFrameCount[base.NPC.type])) * 1f / 2f;
            ...
            Main.spriteBatch.Draw(base.Mod.GetTexture("NPCs/终天灭世眼/天气封印石_Glow"), vector2, new Rectangle?(base.NPC.frame), color * (float)M, base.NPC.rotation, vector, 1f, effects, 0f);
		}
