...
namespace MythMod.NPCs.OceanCrystal
{
    ...
    public class OceanCrystal : ModNPC
	{
            ...
            if (flag1)
            {
                ...
                if (NPC.localAI[0] > 121 && NPC.localAI[0] <= 720)
                {
                    ...
                    if (NPC.localAI[0] % 60 == 0)
                    {
                        ...
                        for (int k = 0; k < 3; k++)
                        {
                            ...
                            Projectile.NewProjectile(base.NPC.Center.X, base.NPC.Center.Y + 35, v.X, v.Y, base.Mod.Find<ModProjectile>("StarFish1").Type, 95, 0.2f, Main.myPlayer, 0f, 0f);
                            NPC.NewNPC((int)base.NPC.Center.X - 10, (int)base.NPC.Center.Y + 43, 67, 0, 0f, 0f, 0f, 0f, 255);
                        }
                        for (int k = 0; k < 3; k++)
                        {
                            Vector2 v = new Vector2(0, Main.rand.Next(0, 2000) / 400f).RotatedByRandom(Math.PI * 2);
                            Projectile.NewProjectile(base.NPC.Center.X, base.NPC.Center.Y + 35, v.X, v.Y, base.Mod.Find<ModProjectile>("StarFish2").Type, 95, 0.2f, Main.myPlayer, 0f, 0f);
                            NPC.NewNPC((int)base.NPC.Center.X - 10, (int)base.NPC.Center.Y + 43, 67, 0, 0f, 0f, 0f, 0f, 255);
                        }
                        for (int k = 0; k < 3; k++)
                        {
                            Vector2 v = new Vector2(0, Main.rand.Next(0, 2000) / 400f).RotatedByRandom(Math.PI * 2);
                            Projectile.NewProjectile(base.NPC.Center.X, base.NPC.Center.Y + 35, v.X, v.Y, base.Mod.Find<ModProjectile>("StarFish3").Type, 95, 0.2f, Main.myPlayer, 0f, 0f);
                        }
                    }
                }
                if (NPC.localAI[0] > 721 && NPC.localAI[0] <= 1440)
                {
                    ...
                    if (NPC.localAI[0] % 15 == 0)
                    {
                        ...
                        Projectile.NewProjectile(base.NPC.Center.X + 45, base.NPC.Center.Y + 45, v.X, v.Y, base.Mod.Find<ModProjectile>("AzureRay").Type, 75, 0.2f, Main.myPlayer, 0f, 0f);
                        Vector2 v2 = (player.Center - (NPC.Center + new Vector2(-25, 45))) * (10 / num3);
                        Projectile.NewProjectile(base.NPC.Center.X - 25, base.NPC.Center.Y + 45, v2.X, v2.Y, base.Mod.Find<ModProjectile>("AzureRay").Type, 75, 0.2f, Main.myPlayer, 0f, 0f);
                    }
                    if (NPC.localAI[0] % 30 == 0)
                    {
                        Vector2 v = (player.Center - (NPC.Center + new Vector2(30, 43))) * (13 / num3);
                        Projectile.NewProjectile(base.NPC.Center.X + 30, base.NPC.Center.Y + 43, v.X, v.Y, base.Mod.Find<ModProjectile>("AzureRay2").Type, 105, 0.2f, Main.myPlayer, 0f, 0f);
                        Vector2 v2 = (player.Center - (NPC.Center + new Vector2(-10, 43))) * (13 / num3);
                        Projectile.NewProjectile(base.NPC.Center.X - 10, base.NPC.Center.Y + 43, v2.X, v2.Y, base.Mod.Find<ModProjectile>("AzureRay2").Type, 105, 0.2f, Main.myPlayer, 0f, 0f);
                    }
                }
                if (NPC.localAI[0] > 1441 && NPC.localAI[0] <= 2520)
                {
                    ...
                    if (NPC.localAI[0] % 150 == 0)
                    {
                        ...
                        int num4 = Projectile.NewProjectile(base.NPC.Center.X + 45, base.NPC.Center.Y + 45, v.X, v.Y, base.Mod.Find<ModProjectile>("Vortex").Type, 137, 0.2f, Main.myPlayer, 0f, 0f);
                ...
                if (NPC.localAI[0] > 3001 && NPC.localAI[0] <= 3600)
                {
                    ...
                    if (NPC.localAI[0] % 45 == 0)
                    {
                        for (int k = 0; k < 2; k++)
                        {
                            ...
                            Projectile.NewProjectile(base.NPC.Center.X + 30, base.NPC.Center.Y, v.X, v.Y, base.Mod.Find<ModProjectile>("OceanCrystalSpice").Type, 90, 0.2f, Main.myPlayer, 0f, 0f);
                            Vector2 v2 = (player.Center - (NPC.Center + new Vector2(-10, 0))).RotatedByRandom(Math.PI * 0.5f) * (17 / num3);
                            Projectile.NewProjectile(base.NPC.Center.X - 10, base.NPC.Center.Y, v2.X, v2.Y, base.Mod.Find<ModProjectile>("OceanCrystalSpice").Type, 90, 0.2f, Main.myPlayer, 0f, 0f);
                        }
                    }
                }
            ...
            if (flag2 || flag3 || flag4)
            {
                if(flag2s)
                {
                    ...
                    for (int k = 0; k < 10; k++)
                    {
                        ...
                        Projectile.NewProjectile(base.NPC.Center.X, base.NPC.Center.Y + 35, v.X, v.Y, base.Mod.Find<ModProjectile>("海洋波纹2").Type, 111, 0.2f, Main.myPlayer, 0f, 0f);
                        Projectile.NewProjectile(base.NPC.Center.X, base.NPC.Center.Y + 35, v2.X, v2.Y, base.Mod.Find<ModProjectile>("AzureRay").Type, 75, 0.2f, Main.myPlayer, 0f, 0f);
                    }
                }
                NPC.localAI[0] += 1;
                if(flag2a)
                ...
                if (NPC.localAI[0] >= 1000 && NPC.localAI[0] < 2200)
                {
                    ...
                    if (NPC.localAI[0] <= 1199 && NPC.localAI[0] % 200 == 50)
                    {
                        ...
                        numk = Projectile.NewProjectile(base.NPC.Center.X, base.NPC.Center.Y, vector.X, vector.Y, base.Mod.Find<ModProjectile>("PoisonRayPurple").Type, 125, 0, Main.myPlayer, 0f, 0f);
                    }
                    if (!flag2b)
                    {
                        if (Main.projectile[numk].type == base.Mod.Find<ModProjectile>("PoisonRayPurple").Type)
                        {
                ...
                if (NPC.localAI[0] >= 2200 && NPC.localAI[0] < 3200)
                {
                    ...
                    if (NPC.localAI[0] % 180 == 50)
                    {
                        for(int i = 0; i < 45; i++)
                        {
                            ...
                            Projectile.NewProjectile(base.NPC.Center.X, base.NPC.Center.Y, vector.X, vector.Y, base.Mod.Find<ModProjectile>("PhantomFish").Type, 88, 0, Main.myPlayer, Main.rand.Next(-200,200) / 100f, 0f);
                        }
                    }
                }
            ...
            if (flag5)
            {
                ...
                if (NPC.localAI[0] % 500 == 250)
                {
                    for (int i = 0; i < 45; i++)
                    {
                        ...
                        Projectile.NewProjectile(base.NPC.Center.X, base.NPC.Center.Y, vector.X, vector.Y, base.Mod.Find<ModProjectile>("空间粒子流").Type, 45, 0, Main.myPlayer, 0, 0f);
                    }
                }
                if (NPC.localAI[0] % 167 == 0)
                {
                    ...
                    {
                        Vector2 v = (player.Center - (NPC.Center + new Vector2(30, 0))).RotatedByRandom(Math.PI * 0.5f) * (17 / num3);
                        Projectile.NewProjectile(base.NPC.Center.X + 30, base.NPC.Center.Y, v.X, v.Y, base.Mod.Find<ModProjectile>("OceanCrystalSpice").Type, 111, 0.2f, Main.myPlayer, 0f, 0f);
                        Vector2 v2 = (player.Center - (NPC.Center + new Vector2(-10, 0))).RotatedByRandom(Math.PI * 0.5f) * (17 / num3);
                        Projectile.NewProjectile(base.NPC.Center.X - 10, base.NPC.Center.Y, v2.X, v2.Y, base.Mod.Find<ModProjectile>("OceanCrystalSpice").Type, 111, 0.2f, Main.myPlayer, 0f, 0f);
                    }
                }
                ...
                if (NPC.localAI[0] % 1429 == 0)
                {
                    for (int k = 0; k < 20; k++)
                    {
                        ...
                        Projectile.NewProjectile(base.NPC.Center.X + 30, base.NPC.Center.Y, v.X, v.Y, base.Mod.Find<ModProjectile>("AttractLightBall").Type, 82, 0.2f, Main.myPlayer, 0f, 0f);
                        Vector2 v2 = (player.Center - (NPC.Center + new Vector2(-10, 0))).RotatedByRandom(Math.PI * 0.5f) * (Main.rand.Next(0,250) / 12f / num3);
                        Projectile.NewProjectile(base.NPC.Center.X - 10, base.NPC.Center.Y, v2.X, v2.Y, base.Mod.Find<ModProjectile>("LeaveLightBall").Type, 82, 0.2f, Main.myPlayer, 0f, 0f);
                    }
                }
            }
        }
		...
		public override void HitEffect(int hitDirection, double damage)
		{
            ...
            for (int i = 0; i < 15; i++)
			{
				Dust.NewDust(base.NPC.position, base.NPC.width, base.NPC.height, Mod.Find<ModDust>("Crystal2").Type, (float)hitDirection, -1f, 0, default(Color), 1f);
			}
			...
			if (base.NPC.life <= 0)
			{
                ...
                Projectile.NewProjectile(base.NPC.Center.X - 10, base.NPC.Center.Y, v2.X, v2.Y, base.Mod.Find<ModProjectile>("OceanSealBroken3").Type, (int)(base.NPC.damage * 1.2f), 0.2f, Main.myPlayer, 0f, 0f);
                Vector2 v3 = new Vector2(Main.rand.Next(0, 15) / 3f, 0).RotatedByRandom(Math.PI * 2) + NPC.velocity;
                Projectile.NewProjectile(base.NPC.Center.X - 10, base.NPC.Center.Y, v3.X, v3.Y, base.Mod.Find<ModProjectile>("OceanSealBroken4").Type, (int)(base.NPC.damage * 1.2f), 0.2f, Main.myPlayer, 0f, 0f);
                float scaleFactor = (float)(Main.rand.Next(-200, 200) / 100f);
                for (int i = 0; i < 120; i++)
                {
                    Dust.NewDust(base.NPC.position, base.NPC.width, base.NPC.height, Mod.Find<ModDust>("Crystal2").Type, (float)hitDirection, -1f, 0, default(Color), 2f);
                }
