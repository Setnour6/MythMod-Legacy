...
namespace MythMod.NPCs.Yuanbao
{
	...
	public class AuYuanbao : ModNPC
	{
		...
		{
            // base.DisplayName.SetDefault("金元宝");
            base.DisplayName.AddTranslation(GameCulture.Chinese, "金元宝");
		}
		...
		public override void HitEffect(NPC.HitInfo hit)
        {
            ...
            if (base.NPC.life <= 0)
            {
                ...
                if(mplayer.GoldTime > 0)
                {
                    if (Main.expertMode)
                    {
                        mplayer.GoldPoint += 400;
                    }
                    else
                    {
                        mplayer.GoldPoint += 200;
                    }
                    if (mplayer.Double > 0)
                    {
                        if (Main.expertMode)
