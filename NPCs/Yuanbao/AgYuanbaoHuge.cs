...
namespace MythMod.NPCs.Yuanbao
{
	...
	public class AgYuanbaoHuge : ModNPC
	{
		private int num1 = 0;
		private int num2;
		...
		{
            base.DisplayName.SetDefault("巨大银元宝");
            base.DisplayName.AddTranslation(GameCulture.Chinese, "巨大银元宝");
		}
		...
		public override void HitEffect(int hitDirection, double damage)
        {
            ...
            if (base.npc.life <= 0)
            {
                ...
                if (mplayer.GoldTime > 0)
                {
                    if (Main.expertMode)
                    {
                        mplayer.GoldPoint += 120;
                    }
                    else
                    {
                        mplayer.GoldPoint += 60;
                    }
                    if (mplayer.Double > 0)
                    {
                        if (Main.expertMode)
