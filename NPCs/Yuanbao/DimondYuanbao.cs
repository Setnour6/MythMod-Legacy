...
namespace MythMod.NPCs.Yuanbao
{
	...
	public class DimondYuanbao : ModNPC
	{
		private int num1 = 0;
		private int num2;
		...
		public override void SetStaticDefaults()
		{
            ...
            base.DisplayName.AddTranslation(GameCulture.Chinese, "钻石元宝");
		}
		...
		public override void HitEffect(int hitDirection, double damage)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                Dust.NewDust(base.NPC.position, base.NPC.width, base.NPC.height, 13, (float)hitDirection, -1f, 0, default(Color), 1f);
            }
            for (int j = 0; j < 3; j++)
            {
                Dust.NewDust(base.NPC.position, base.NPC.width, base.NPC.height, 13, (float)hitDirection, -1f, 0, default(Color), 1f);
            ...
            if (base.NPC.life <= 0)
            {
                ...
                {
                    Item.NewItem((int)base.NPC.Center.X, (int)base.NPC.Center.Y, base.NPC.width, base.NPC.height, base.Mod.Find<ModItem>("CYuanbao").Type, 1, false, 0, false, false);
                }
                ...
                Gore.NewGore(base.NPC.position, base.NPC.velocity * scaleFactor, base.Mod.GetGoreSlot("Gores/钻石元宝碎块1"), 1f);
                Gore.NewGore(base.NPC.position, base.NPC.velocity * scaleFactor, base.Mod.GetGoreSlot("Gores/钻石元宝碎块2"), 1f);
                Gore.NewGore(base.NPC.position, base.NPC.velocity * scaleFactor, base.Mod.GetGoreSlot("Gores/钻石元宝碎块2"), 1f);
                Gore.NewGore(base.NPC.position, base.NPC.velocity * scaleFactor, base.Mod.GetGoreSlot("Gores/钻石元宝碎块3"), 1f);
                if (mplayer.GoldTime > 0)
                {
                    if(Main.expertMode)
                    {
                        mplayer.GoldPoint += 66666;
                    }
                    else
                    {
                        mplayer.GoldPoint += 33333;
                    }
                    if (mplayer.Double > 0)
                    {
                        if (Main.expertMode)
