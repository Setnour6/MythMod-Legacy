...
namespace MythMod.NPCs.Yuanbao
{
	...
	public class AuYuanbaoHuge : ModNPC
	{
		private int num1 = 0;
		private int num2;
		...
		{
            // base.DisplayName.SetDefault("巨大金元宝");
            base.DisplayName.AddTranslation(GameCulture.Chinese, "巨大金元宝");
		}
		...
		public override void HitEffect(NPC.HitInfo hit)
        {
            ...
            if (base.NPC.life <= 0)
            {
                ...
                {
                    Item.NewItem((int)base.NPC.Center.X, (int)base.NPC.Center.Y, base.NPC.width, base.NPC.height, base.Mod.Find<ModItem>("AuYuanbao").Type, 1, false, 0, false, false);
                }
                ...
                Gore.NewGore(base.NPC.position, base.NPC.velocity * scaleFactor, base.Mod.GetGoreSlot("Gores/金元宝碎块1"), 1f);
                Gore.NewGore(base.NPC.position, base.NPC.velocity * scaleFactor, base.Mod.GetGoreSlot("Gores/金元宝碎块2"), 1f);
                Gore.NewGore(base.NPC.position, base.NPC.velocity * scaleFactor, base.Mod.GetGoreSlot("Gores/金元宝碎块2"), 1f);
                Gore.NewGore(base.NPC.position, base.NPC.velocity * scaleFactor, base.Mod.GetGoreSlot("Gores/金元宝碎块3"), 1f);
                Gore.NewGore(base.NPC.position, base.NPC.velocity * scaleFactor, base.Mod.GetGoreSlot("Gores/金元宝碎块1"), 1f);
                Gore.NewGore(base.NPC.position, base.NPC.velocity * scaleFactor, base.Mod.GetGoreSlot("Gores/金元宝碎块2"), 1f);
                Gore.NewGore(base.NPC.position, base.NPC.velocity * scaleFactor, base.Mod.GetGoreSlot("Gores/金元宝碎块2"), 1f);
                Gore.NewGore(base.NPC.position, base.NPC.velocity * scaleFactor, base.Mod.GetGoreSlot("Gores/金元宝碎块3"), 1f);
                if (mplayer.GoldTime > 0)
                {
                    if (Main.expertMode)
                    {
                        mplayer.GoldPoint += 4000;
                    }
                    else
                    {
                        mplayer.GoldPoint += 2000;
                    }
                    if (mplayer.Double > 0)
                    {
                        if (Main.expertMode)
