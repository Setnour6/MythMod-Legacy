...
namespace MythMod.NPCs
{
    ...
    public class MachineLaser : ModNPC
	{
        private bool A = true;
        private int num10;
        private int num11;
        ...
        public override void AI()
        {
            ...
            if (!dayTime)
            {
                Dust.NewDust(new Vector2((float)base.npc.Center.X - 30f, (float)base.npc.Center.Y - 24), 8, 48, 6, 0f, 0f, 0, default(Color), 1f);
                base.npc.rotation = (float)Math.PI * 1.5f;
                if (base.npc.localAI[0] > 45)
                {
                    npc.position = Main.screenPosition + new Vector2(0, Main.mouseY);
                }
                else
                {
                    npc.velocity = (Main.screenPosition + new Vector2(0, Main.mouseY) - npc.Center) * (25f / ((Main.screenPosition + new Vector2(0, Main.mouseY) - npc.Center).Length() + 0.01f));
                }
