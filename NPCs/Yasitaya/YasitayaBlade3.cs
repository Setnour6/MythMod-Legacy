    ...
    public class YasitayaBlade3 : ModProjectile
    {
            ...
            if (n != -1)
            {
                ...
                if (projectile.timeLeft < 601)
                {
                    ...
                    if (projectile.timeLeft % 100 == 50)
                    {
                        ...
                        Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, vp.X, vp.Y, mod.ProjectileType("BloodBlade"), 90, 0f, Main.myPlayer, Main.rand.Next(6, 48), 0f);
                    }
                }
            }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            if(!Main.gamePaused)
            {
                ...
                if (K < 100)
                {
                    Projectile.NewProjectile(p.Center.X, p.Center.Y, 0, 0, mod.ProjectileType("Hit"), projectile.damage, 4, 255, 0f, 0f);
                }
            }
            return false;
        }
