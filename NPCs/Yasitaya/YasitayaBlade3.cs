...
{
    public class YasitayaBlade3 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (n != -1)
            {
                ...
                if (Projectile.timeLeft < 601)
                {
                    ...
                    if (Projectile.timeLeft % 100 == 50)
                    {
                        Vector2 vp = (Projectile.Center - Main.npc[n].Center) / (Projectile.Center - Main.npc[n].Center).Length();
                        Projectile.NewProjectile(Projectile.Center.X, Projectile.Center.Y, vp.X, vp.Y, Mod.Find<ModProjectile>("BloodBlade").Type, 90, 0f, Main.myPlayer, Main.rand.Next(6, 48), 0f);
                    }
                }
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if(!Main.gamePaused)
            {
                float K = (p.Center - Projectile.Center).Length();
                if (K < 100)
                {
                    Projectile.NewProjectile(p.Center.X, p.Center.Y, 0, 0, Mod.Find<ModProjectile>("Hit").Type, Projectile.damage, 4, 255, 0f, 0f);
                }
            }
            return false;
        }
